...
var (
	_ = jumbochain.ChainReader(&Client{})
	_ = jumbochain.TransactionReader(&Client{})
	_ = jumbochain.ChainStateReader(&Client{})
	_ = jumbochain.ChainSyncReader(&Client{})
	_ = jumbochain.ContractCaller(&Client{})
	_ = jumbochain.GasEstimator(&Client{})
	_ = jumbochain.GasPricer(&Client{})
	_ = jumbochain.LogFilterer(&Client{})
	_ = jumbochain.PendingStateReader(&Client{})
	// _ = jumbochain.PendingStateEventer(&Client{})
	_ = jumbochain.PendingContractCaller(&Client{})
)
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == jumbochain.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != jumbochain.NotFound {
		t.Fatal("error should be jumbochain.NotFound")
	}
}
