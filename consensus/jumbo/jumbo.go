...
import (
	...
	cmath "jumbochain.org/common/math"
...
type Jumbo struct {
	...
	config      *params.JumboConsensusConfig // Consensus engine configuration parameters for parlia consensus
...
func (p *Jumbo) IsSystemTransaction(tx *types.Transaction, header *types.Header) (bool, error) {
	// deploy a contract
	if tx.To() == nil {
		return false, nil
	}
...
func (p *Jumbo) IsSystemContract(to *common.Address) bool {
	if to == nil {
		return false
	}
...
func (p *Jumbo) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (p *Jumbo) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (p *Jumbo) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (p *Jumbo) getParent(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) (*types.Header, error) {
...
func (p *Jumbo) verifyVoteAttestation(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Jumbo) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Jumbo) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Jumbo) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (p *Jumbo) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (p *Jumbo) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
...
// from.
func (p *Jumbo) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Jumbo) prepareValidators(header *types.Header) error {
...
func (p *Jumbo) assembleVoteAttestation(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (p *Jumbo) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (p *Jumbo) verifyValidators(header *types.Header) error {
	if header.Number.Uint64()%p.config.Epoch != 0 {
		return nil
	}
...
func (p *Jumbo) distributeFinalityReward(chain consensus.ChainHeaderReader, state *state.StateDB, header *types.Header,
...
// rewards given.
func (p *Jumbo) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction,
...
func (p *Jumbo) IsActiveValidatorAt(chain consensus.ChainHeaderReader, header *types.Header) bool {
	number := header.Number.Uint64()
	snap, err := p.snapshot(chain, number-1, header.ParentHash, nil)
	if err != nil {
...
func (p *Jumbo) VerifyVote(chain consensus.ChainHeaderReader, vote *types.VoteEnvelope) error {
...
// with.
func (p *Jumbo) Authorize(val common.Address, signFn SignerFn, signTxFn SignerTxFn) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *Jumbo) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
...
func (p *Jumbo) shouldWaitForCurrentBlockProcess(chain consensus.ChainHeaderReader, header *types.Header, snap *Snapshot) bool {
	if header.Difficulty.Cmp(diffInTurn) == 0 {
		return false
	}
...
func (p *Jumbo) EnoughDistance(chain consensus.ChainReader, header *types.Header) bool {
...
func (p *Jumbo) AllowLightProcess(chain consensus.ChainReader, currentHeader *types.Header) bool {
...
func (p *Jumbo) IsLocalBlock(header *types.Header) bool {
	return p.val == header.Coinbase
}

func (p *Jumbo) SignRecently(chain consensus.ChainReader, parent *types.Block) (bool, error) {
...
// current signer.
func (p *Jumbo) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (p *Jumbo) SealHash(header *types.Header) (hash common.Hash) {
...
func (p *Jumbo) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "jumbo",
		Version:   "1.0",
...
func (p *Jumbo) getCurrentValidators(blockHash common.Hash, blockNum *big.Int) ([]common.Address, map[common.Address]*types.BLSPublicKey, error) {
	// block
...
func (p *Jumbo) distributeIncoming(val common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
...
// slash spoiled validators
func (p *Jumbo) slash(spoiledVal common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// method
	method := "slash"
...
// init contract
func (p *Jumbo) initContract(state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// method
	method := "init"
	// contracts
...
func (p *Jumbo) distributeToSystem(amount *big.Int, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// get system message
...
// slash spoiled validators
func (p *Jumbo) distributeToValidator(amount *big.Int, validator common.Address,
...
// get system message
func (p *Jumbo) getSystemMessage(from, toAddress common.Address, data []byte, value *big.Int) callmsg {
	return callmsg{
		jumbochain.CallMsg{
			From:     from,
...
func (p *Jumbo) GetJustifiedNumberAndHash(chain consensus.ChainHeaderReader, header *types.Header) (uint64, common.Hash, error) {
...
func (p *Jumbo) GetFinalizedHeader(chain consensus.ChainHeaderReader, header *types.Header) *types.Header {
	if chain == nil || header == nil {
		return nil
	}
	// if !chain.Config().IsPlato(header.Number) {
...
func (p *Jumbo) backOffTime(snap *Snapshot, header *types.Header, val common.Address) uint64 {
	if snap.inturn(val) {
		return 0
	} else {
